...
class MVTecDataset(Dataset):
    def __init__(
        ...
        texture_source_dir: str = "", structure_grid_size = 8,
...
class MVTec3DDataset(Dataset):
    def __init__(
        ...
        self.datadir = datadir
        self.target = target
        self.file_list = glob(os.path.join(self.datadir, self.target, 'train/good/rgb/*' if train else 'test/*/rgb/*'))
...
class MVTec_3D_pretrained(Dataset):
    def __init__(
        self, datadir: str, target: str, 
        resize: Tuple[int, int] = (256,256),
        texture_source_dir: str = "", structure_grid_size = 8,
        transparency_range: List[float] = [0.15, 1.],
        perlin_scale: int = 6, min_perlin_scale: int = 0, perlin_noise_threshold: float = 0.5,
        ...
        self.datadir = datadir
        self.target = target
        good_list = glob(os.path.join(self.datadir, self.target, 'train/good/rgb/*'))
        test_list = glob(os.path.join(self.datadir, self.target, 'test/*/rgb/*'))
        self.file_list = good_list+test_list
